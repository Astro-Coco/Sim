...
def newton(f, guess=1):
    ...
    while (abs(dx) >= tol) & (n < N):
        F = f(x)
        dxi = max([abs(eps * x), eps])
        dF = 0.5 * (f(x + dxi) - f(x - dxi)) / dxi
        dx = -1 * F / dF
...
class nozzle:
    ...
    def find_mach(self, k):
        ...
        def f_expansion_ratio(m):
            ...
            expansion_ratio_eq = (k/2+0.5)**(-(k/2+0.5)/(k-1)) * 1/m*(1+(k/2-0.5)*m**2)**((k/2+0.5)/(k-1))
    ...
    def calculate_cf_pepbratio(self, pbpc_ratio, heat_capacity_ratio):
        ...
        self.find_mach(k)
        pept_ratio = (1 + 0.5*(k - 1)*self.mach ** 2)**(-k/(k - 1))  # Exit pressure/total pressure ratio (note : Pt = Pc)

        # Calculate thrust coefficient and pepb ratio
        cf = k*np.sqrt(2/(k - 1)*(2/(k + 1)) ** ((k + 1)/(k - 1)) * (1-pept_ratio ** (1 - 1/k))) + \
...
class chamber:
    def __init__(self, initial_port_radius, final_port_radius, grain_length, fuel, efficiency):
        # Chamber objects
    ...
    def m_dot_fuel(self, m_dot_ox):
        ...
        return fuel_inside_area * self.fuel['density'] * r_dot.real
...
class tubing:
    ...
    def get_p_loss(self, m_dot_ox, p_tank):
        p_loss = m_dot_ox * 175 * 6894.76 / 2.2
        x = m_dot_ox * 0.1 / 2.2
        return p_loss, x
...
class hybrid_motor:
    def __init__(self, tank, tubing, injector, chamber, nozzle):
        self.tank = tank
        self.tubing = tubing
    ...
    def solve_cs(self, guess_pc=100 * 6894.76, guess_mdo=2):
        ...
        def f_mox(pc, m_dot_ox):
            ...
            p_loss, x_loss = self.tubing.get_p_loss(m_dot_ox, self.tank.P)  # Pressure loss by tubing and vapor fraction
            self.injector.set_0(self.tank.P - p_loss, x_loss)  # Assign pressure before injector
            m_dot_ox_i, po = self.injector.inj()  # m_dot_ox and current pressure after injector
        ...
        def f_cs(pc, m_dot_ox):
            ...
            cs = self.chamber.efficiency * quick_cs(pc, of, self.chamber.fuel['c'])  # estimated C* with CEA
    ...
    def burn(self, prints=False):
        ...
        while (self.tank.ml > 0) & (self.chamber.r < self.chamber.r_final):
            ...
            of = self.injector.m_dot / self.chamber.m_dot_fuel(self.injector.m_dot)  # O/F
            cs = self.chamber.efficiency * quick_cs(self.chamber.pressure_chamber, of,
                                                           self.chamber.fuel['c'])  # C*
            heat_capacity_ratio = 1.3  # to modify for a new calculation with CEA
            cf, pepb = self.nozzle.calculate_cf_pepbratio(self.back_pressure/self.chamber.pressure_chamber,
                                                          heat_capacity_ratio)  # CF and pe/pb
            ...
            self.data['P comb (psia)'].append(self.chamber.pressure_chamber / 6894.76)
            self.data['P inj. (psia)'].append(self.injector.p0 / 6894.76)
            self.data['P tank (psia)'].append(self.tank.P / 6894.76)
            ...
            self.data['Gox (kg/s-m2)'].append(self.injector.m_dot / (np.pi * self.chamber.r ** 2))
            self.data['P crit (psia)'].append(self.injector.po / 6894.76)
            ...
            if prints:
                ...
                progr = (self.chamber.r - self.chamber.r_init) / (self.chamber.r_final - self.chamber.r_init)
        ...
        if prints:
            ...
            if self.tank.ml < 0:  # No more oxidizer
                print('Oxidizer tank empty\n' + 'Final grain radius', '{:.5f}'.format(self.chamber.r),
                      'm [' + '{:.1f}'.format(100 * (self.chamber.r - self.chamber.r_init) / (
                              self.chamber.r_final - self.chamber.r_init)), '% used]')
            else:  # No more fuel
                print('Fuel grain exhausted\n' + 'Liquid oxidizer mass left:', '{:.3f}'.format(self.tank.ml),
                      'kg [' + '{:.1f}'.format(100 * (ml0 - self.tank.ml) / ml0), '% used]')
    ...
    def plot(self):
        ...
        fig, ax = plt.subplots()
        ax.plot(self.data['Time      (s)'], self.data['P tank (psia)'], color='tab:blue', label='Tank')
        ax.plot(self.data['Time      (s)'], self.data['P inj. (psia)'], color='k', label='Inj.')
        ax.plot(self.data['Time      (s)'], self.data['P comb (psia)'], color='tab:red', label='Comb.')
        ax.plot(self.data['Time      (s)'], self.data['P crit (psia)'], color='tab:orange', label='Crit.')
        ...
        ax01 = ax[1].twinx()
        ax01.plot(self.data['Time      (s)'], self.data['O/F       (-)'], color='tab:blue')
        ax01.set_ylabel('O/F')
        ...
        fig, ax = plt.subplots(2)
        ax[0].plot(self.data['Time      (s)'], np.array(self.data['Thrust    (N)']) / 1e3, color='k')
        ax[0].set_ylabel('Thrust (kN)')
        ax[0].grid(True)
        ax[1].plot(self.data['Time      (s)'], self.data['ISP     (m/s)'], color='tab:orange', label='ISP')
        ax[1].plot(self.data['Time      (s)'], self.data['C*      (m/s)'], color='tab:red', label='C*')
    ...
    def print(self):
        ...
        for key, value in self.data.items():
            if key != 'Time      (s)' and key != 'P crit (psia)':  # No needs to print time and Pcrit
                if key == 'Impulse  (Ns)':  # Only last value for impulse
                    print(f'| {key} |     -     |     -     | {value[-1]:5.3e} |')
                else:
                    value = min_mean_max(value)  # Min, average and max
                    print(f'| {key} | {value[0]:5.3e} | {value[1]:5.3e} | {value[2]:5.3e} |')
        print('|---------------|-----------|-----------|-----------|')

# Motor definition fron json file
def mot_from_json(prm):
    tank = nox_tank(volume=prm['tank']['v'], ullage=prm['tank']['x'], pressure=prm['tank']['p'])
    ...
    chamb = chamber(initial_port_radius=prm['chamber']['ri'], final_port_radius=prm['chamber']['re'],
                    grain_length=prm['chamber']['l'], fuel=prm['chamber']['fuel'],
                    efficiency=prm['chamber']['efficiency'])
    noz = nozzle(throat_area=prm['nozzle']['at'], expansion_ratio=prm['nozzle']['expan'])

    return hybrid_motor(tank, tub, inj, chamb, noz)
